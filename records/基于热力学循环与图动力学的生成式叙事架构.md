...
**不是LLM调用WM，也不是WM调用LLM，而是它们在Director的协调下相互询问。**
...
1.  **叙事冲动**：Actor (LLM) 基于上下文产生一个意图向量 $\vec{I}$ 或 意图图（Intent Graph）（例如：“Alice 试图攻击 Bob”）。
...
4.  **去噪与坍缩 (Cooling)**：Simulator 在 $\vec{I}$ 的引导下进行去噪。
    ...
    *   WM 遇到难以推理的情况时，可以让 LLM 利用其强大的符号推理能力计算出结果辅助 WM 推理。
    *   **输出**：演化后的子图状态 $G'_{sub}$。
...
受 [**Nested Learning: The Illusion of Deep Learning Architectures**](https://abehrouz.github.io/files/NL.pdf) 的启发，也许我们可以将图以一个庞大的神经网络的形式并入 Director 模块，Director同时成为一个巨大的 key-value 数据库。Director 将来自LLM的 意图向量 $\vec{I}$ 输入变为状态向量的输出。将 Director 输出的状态向量输入 WM 进行加噪去噪处理，将处理后的状态向量分别输入 LLM 和 Director。LLM 利用这一条与意图向量 $\vec{I}$对应的状态向量生成文本，Director 则将该状态学习并记忆，以供下一次查询。（补充：如果担心一条向量不足以表达复杂的状态，Director 返回的完全可以是状态向量序列，Director 自回归地生成。）
