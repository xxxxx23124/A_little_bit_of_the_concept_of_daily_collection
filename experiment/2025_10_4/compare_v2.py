...
if __name__=='__main__':
    ...
    hyper_model_v2 = train_model(hyper_model_v2, X_train, y_train, epochs=30000, lr=0.0005)
    ...
    with torch.no_grad():
        device = next(hyper_model_v2.parameters()).device  # 获取模型所在的设备
        X_train_gpu = X_train.to(device)
        predicted_hyper_v2 = hyper_model_v2(X_train_gpu)
    ...
    plt.plot(X_train.numpy(), predicted_hyper_v2.cpu().numpy(), 'g--', label=f'HyperNetV2 Fit ({hyper_v2_params:,} params)', linewidth=2)
    plt.legend()
    plt.grid(True)
