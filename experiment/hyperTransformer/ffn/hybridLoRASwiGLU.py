...
from experiment.hyperTransformer.linear.hyperLoRALinear import HyperLoRALinear
...
class HybridLoRASwiGLU(BaseSwiGLU):
    ...
    def _init_sublayers(self, input_dim, output_dim, up_proj_dim, **kwargs):
        ...
        # 检查必需的参数是否存在
        if any(p is None for p in [compressed_feature_dim, rank]):
            raise ValueError(
                "HybridLoRASwiGLU requires 'compressed_feature_dim' and 'rank' to be provided in kwargs."
            )
