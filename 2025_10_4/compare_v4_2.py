...
if __name__ == '__main__':
    ...
    SWIGLU_LAYER_SIZES = [1, 48, 48, 1]  # 使用和HyperNet主网络相同的层级结构
    ...
    HIDDEN_DIM_MULTIPLIER = hyper_v4_params / params_per_multiplier
    ...
    print("\n--- Training HyperNetV4 ---")
    hyper_model_v4 = train_model(hyper_model_v4, X_train, y_train, epochs=EPOCHS, lr=LR)
    ...
    plt.title("Fair Comparison: Dynamic Gating (HyperNetV4) vs. Static Gating (SwiGLU)")
    ...
    plt.plot(X_train.numpy(), predicted_hyper_v4.cpu().numpy(), 'g--',
             label=f'HyperNetV4 Fit ({hyper_v4_params:,} params)', linewidth=2)
    plt.legend()
    plt.grid(True)
    plt.ylim(-1.5, 1.5)
    plt.show()
