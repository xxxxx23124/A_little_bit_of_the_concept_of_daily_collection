...
class HybridSwiGLU(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, gate_dim, rank, ratio_dim):
        ...
        self.static_up_proj = nn.Linear(input_dim, hidden_dim)

        # 2. 动态门控 (Dynamic Gating)
        # 注意：门控作用于升维后的hidden_dim
        self.dynamic_gate_generator = HyperGate(input_dim, hidden_dim, gate_dim, rank, ratio_dim)
        self.swish = nn.SiLU()
...
class HyperNetV5(nn.Module):
    def __init__(self, layer_sizes, hidden_dim, gate_dim, rank, ratio_dim):
        ...
        for i in range(len(layer_sizes) - 1):
            ...
            self.hybrid_ffn_list.append(
                HybridSwiGLU(input_dim, output_dim, hidden_dim, gate_dim, rank, ratio_dim)
            )
